...
public class TileEntityGrill extends TileEntity implements IInventory, ITickable {
	...
	public void update() {
		...
		if (!this.world.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				BlockGrill.setState(this.isBurning(), this.world, this.pos);
			}
		}
		if (flag1)
			this.markDirty();
	...
	private boolean canSmelt() {
		if (((ItemStack) this.inventory.get(0)).isEmpty() || ((ItemStack) this.inventory.get(1)).isEmpty())
			...
			ItemStack result = Grilling.INSTANCE.getGrillingResult((ItemStack) this.inventory.get(0), (ItemStack) this.inventory.get(1));
			if (result.isEmpty())
				return false;
			else {
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			ItemStack result = Grilling.INSTANCE.getGrillingResult(input1, input2);
			ItemStack output = (ItemStack) this.inventory.get(3);
