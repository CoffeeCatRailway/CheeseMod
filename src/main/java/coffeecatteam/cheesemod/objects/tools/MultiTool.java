...
public class MultiTool extends ItemTool {
	private static final Set<Block> EFFECTIVE_ON = Sets.newHashSet(Blocks.ACTIVATOR_RAIL, Blocks.COAL_ORE,
			...
			Blocks.CONCRETE_POWDER, Blocks.PLANKS, Blocks.BOOKSHELF, Blocks.LOG, Blocks.LOG2, Blocks.CHEST,
			Blocks.PUMPKIN, Blocks.LIT_PUMPKIN, Blocks.MELON_BLOCK, Blocks.LADDER, Blocks.WOODEN_BUTTON,
			Blocks.WOODEN_PRESSURE_PLATE, InitBlock.CHEESE_METAL_BLOCK, InitBlock.CHEESE_METAL_ORE,
			InitBlock.GRILLED_CHEESE_METAL_BLOCK, InitBlock.GRILLED_CHEESE_METAL_ORE, InitBlock.HAM_RAW_METAL_BLOCK,
			InitBlock.HAM_RAW_METAL_ORE, InitBlock.HAM_COOKED_METAL_BLOCK, InitBlock.HAM_COOKED_METAL_ORE);
	private final Item.ToolMaterial material;

	public MultiTool(String name, ToolMaterial material) {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		else if (facing != EnumFacing.DOWN && world.getBlockState(pos.up()).getMaterial() == Material.AIR
				&& block == Blocks.GRASS) {
			IBlockState state1 = Blocks.GRASS_PATH.getDefaultState();
			world.playSound(player, pos, SoundEvents.ITEM_SHOVEL_FLATTEN, SoundCategory.BLOCKS, 1.0F, 1.0F);
			if (!world.isRemote) {
				world.setBlockState(pos, state1, 11);
