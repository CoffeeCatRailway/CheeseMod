...
public class WorldGenTreeHamRaw extends WorldGenAbstractTree {
	...
	public static final IBlockState LEAVES = InitBlock.CHEESE_LEAVES.getDefaultState()
			.withProperty(BlockCheeseLeaves.VARIANT, EnumHandler.EnumWoodType.HAM_RAW);

	private int minHeight;

	public WorldGenTreeHamRaw() {
		super(false);
		this.minHeight = 12;
	}
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		else
		{
			...
			boolean isSoil = state.getBlock().canSustainPlant(state, world, down, EnumFacing.UP, (BlockCheeseSapling)InitBlock.CHEESE_SAPLING);
	...
	protected boolean canGrowInto(Block blockType) {
		...
		return material == Material.AIR || material == Material.LEAVES || material == Material.GROUND
				...
				|| blockType == InitBlock.FOOD_BLOCK.getDefaultState()
						.withProperty(BlockCheesePlanks.VARIANT, EnumHandler.EnumWoodType.HAM_RAW).getBlock()
				|| blockType == InitBlock.FOOD_BLOCK.getDefaultState()
						.withProperty(BlockCheesePlanks.VARIANT, EnumHandler.EnumWoodType.HAM_COOKED).getBlock();
	}
}
