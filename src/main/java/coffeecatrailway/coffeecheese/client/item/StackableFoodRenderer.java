package coffeecatrailway.coffeecheese.client.item;
...
public class StackableFoodRenderer extends ItemStackTileEntityRenderer {
    ...
    private float getRotateAngle(ListNBT ingredients, int ingredient) {
        return ANGLES[(int) CheeseMod.map(ingredient, 0, ingredients.size(), 0, Math.min(ANGLES.length, CheeseMod.SERVER_CONFIG.stackableFoodIngredientCount.get()))];
    }
    ...
    public void func_239207_a_(ItemStack stack, ItemCameraTransforms.TransformType transform, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        super.func_239207_a_(stack, transform, matrix, buffer, combinedLight, combinedOverlay);
