 ...
 * Created: 25/07/2020
 */
public class FoodBoatEntity extends BoatEntity {

    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(FoodBoatEntity.class, DataSerializers.VARINT);
    ...
    public FoodBoatEntity(EntityType<? extends FoodBoatEntity> type, World world) {
        super(type, world);
        this.preventEntitySpawning = true;
    }

    public FoodBoatEntity(World world, double x, double y, double z) {
        this(CheeseEntities.BOAT.get(), world);
    ...
    public FoodBoatEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
        this(CheeseEntities.BOAT.get(), world);
    }
    ...
    protected void registerData() {
        ...
        this.dataManager.register(BOAT_TYPE, Type.CHEESE.ordinal());
    ...
    public Item getItemBoat() {
        switch (this.getBoatModel()) {
            case CHEESE:
            default:
                return CheeseItems.BOAT_CHEESE.get();
            case GRILLED_CHEESE:
                return CheeseItems.BOAT_GRILLED_CHEESE.get();
            case HAM_RAW:
                return CheeseItems.BOAT_HAM_RAW.get();
            case HAM_COOKED:
                return CheeseItems.BOAT_HAM_COOKED.get();
        }
    }
    ...
    public float getBoatGlide() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
        AxisAlignedBB axisalignedbb1 = new AxisAlignedBB(axisalignedbb.minX, axisalignedbb.minY - .001d, axisalignedbb.minZ, axisalignedbb.maxX, axisalignedbb.minY, axisalignedbb.maxZ);
    ...
    private boolean checkInWater() {
        ...
        int l = MathHelper.ceil(axisalignedbb.minY + .001d);
        int i1 = MathHelper.floor(axisalignedbb.minZ);
        int j1 = MathHelper.ceil(axisalignedbb.maxZ);
        boolean flag = false;
    ...
    private Status getUnderwaterStatus() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
        double d0 = axisalignedbb.maxY + .001d;
        int i = MathHelper.floor(axisalignedbb.minX);
        int j = MathHelper.ceil(axisalignedbb.maxX);
        int k = MathHelper.floor(axisalignedbb.maxY);
    ...
    public void updatePassenger(Entity passenger) {
        if (this.isPassenger(passenger)) {
            ...
            Vector3d vec3d = (new Vector3d(f, 0d, 0d)).rotateYaw(-this.rotationYaw * ((float) Math.PI / 180f) - ((float) Math.PI / 2f));
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != FoodBoatEntity.Status.ON_LAND) {
    ...
    public enum Type {
        CHEESE(CheeseBlocks.CHEESE_PLANKS.get(), "cheese"),
        GRILLED_CHEESE(CheeseBlocks.GRILLED_CHEESE_PLANKS.get(), "grilled_cheese"),
        HAM_RAW(CheeseBlocks.HAM_RAW_PLANKS.get(), "ham_raw"),
        HAM_COOKED(CheeseBlocks.HAM_COOKED_PLANKS.get(), "ham_cooked");
